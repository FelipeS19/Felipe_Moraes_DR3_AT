...
const Dashboard = () => {
  ...
  const { user } = useAuth();
  const { favorites, toggleFavorite } = useFavorites(); // Usa o contexto de favoritos
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={async () => {
        ...
        setIsFavorited(favorites.some(fav => fav.id === item.id)); 
    ...
    <View style={styles.container}>
      ...
      <View style={styles.listContainer}>
        ...
        {Object.keys(groupedItems).map((categoria) => (
          <View key={categoria}>
            <TouchableOpacity onPress={() => navigation.navigate('Categoria', { categoria })}>
              ...
              {selectedItem && (
                <>
                  ...
                  <View style={styles.modalfunction}>
                    ...
                    <CustomButton title="ver mais" style={styles.maisinfo} onPress={() => {
                      navigation.navigate('Detalhes', { item: selectedItem }),
                        setModalVisible(false);
                    }} />

                    <TouchableOpacity style={styles.fav} onPress={handleToggleFavorite}>
                      <Icon name="heart" size={24} color={isFavorited ? "red" : "black"} />
                    </TouchableOpacity>
